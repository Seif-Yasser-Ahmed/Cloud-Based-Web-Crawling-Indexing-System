...
indexing, then heartbeating into `heartbeats` under thread-name.
"""

import os, time, logging, threading, json, hashlib
    ...
    def vis_hb():
        while not stop_vis.wait(HEARTBEAT_INTERVAL):
            try:
                ...
                logger.exception("extend visibility failed")
    ...
    def thr_hb():
        while not stop_hb.wait(HEARTBEAT_INTERVAL):
            try:
                ...
                logger.exception("send heartbeat failed")
    threading.Thread(target=thr_hb, daemon=True).start()

    try:
        # initial heartbeat
        send_heartbeat('indexer')

        # compute page_url_hash
        ...
        rows = [(job_id, url, url_hash, term, freq)
                for term, freq in freqs.items()]

        insert_sql = """
            INSERT INTO index_entries (job_id, page_url, page_url_hash, term, frequency)
            VALUES (%s,%s,%s,%s,%s)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE jobs SET indexed_count = indexed_count + 1 WHERE job_id=%s",
                        (job_id,))
        conn.commit(); conn.close()
        ...
        logger.exception("index_task error %s", url)
