...
def crawl_task(msg):
    ...
    try:
        ...
        if rp is None:
            ...
            try: rp.read()
            except: logger.warning("Could not read robots.txt for %s", origin)
        ...
        for i in range(1, MAX_RETRIES+1):
            try:
                ...
                backoff = 2**(i-1)
                logger.warning("Fetch error %s (attempt %d), retry in %ds", e, i, backoff)
                time.sleep(backoff)
        if not success:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if not ALLOW_EXTERNAL and p.netloc != seed_netloc: continue
        ...
        logger.info("Crawled %s (depth %d), discovered %d links", url, depth, len(children))
