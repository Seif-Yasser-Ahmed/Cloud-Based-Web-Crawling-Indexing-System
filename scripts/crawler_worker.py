...
def crawl_task(msg):
    ...
    try:
        ...
        for attempt in range(1, MAX_RETRIES+1):
            try:
                r = requests.get(url, headers={'User-Agent': 'CrawlerWorker'}, timeout=10)
                r.raise_for_status()
                html, success = r.text, True
                break
            except Exception as e:
                backoff = 2**(attempt-1)
                logger.warning("Fetch error %s (attempt %d), retry in %ds", e, attempt, backoff)
                time.sleep(backoff)
        if not success:
...
def main():
    ...
    logger.info("Starting crawler_worker (min=%d,max=%d)", MIN_THREADS, MAX_THREADS)
    while True:
